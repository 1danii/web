...
export function Accordions() {
  return (
    <div className="flex h-64 w-full justify-center bg-neutral-800">
      <Accordion.Root
...
function AccordionItem(props: PropsWithChildren) {
  return (
    <Accordion.Item className="text-background bg-white duration-[550ms] ease-(--spring) first:rounded-t-2xl last:rounded-b-2xl data-[open]:rounded-2xl data-[open]:not-first:mt-2 data-[open]:not-last:mb-2">
      <Accordion.Header>
        <Accordion.Trigger className="group flex h-9 w-full items-center justify-between px-3 text-sm font-medium">
          Ahoj
          <ChevronDownIcon className="size-3 stroke-black stroke-1 transition-transform duration-[200ms] ease-in-out group-data-[panel-open]:rotate-180" />
        </Accordion.Trigger>
      </Accordion.Header>
      <Accordion.Panel className="h-[var(--accordion-panel-height)] overflow-hidden text-sm text-neutral-500 transition-[height] duration-[550ms] ease-(--spring) data-[ending-style]:h-0 data-[starting-style]:h-0">
        <div className="px-3 pb-2">
          Eu elit nulla culpa et incididunt Lorem est laboris deserunt.
        </div>
      </Accordion.Panel>
    </Accordion.Item>
  );
